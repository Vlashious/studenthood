...
public class StudentTable extends Composite {
    ...
    public StudentTable(Composite parent, int style, Controller controller) {
        ...
        nextPageButton.addListener(SWT.MouseDown, new Listener() {
            ...
            public void handleEvent(Event e) {
                if (searchStudentList.isEmpty()) {
                    try {
                        if (pageNum + 1 < Math
                                .ceil((double) controller.getAllStudentsSize() / numOfStudentsOnPage)) {
                            pageNum++;
                            try {
                                updateTable();
        ...
        lastPageButton.addListener(SWT.MouseDown, new Listener() {
            ...
            public void handleEvent(Event e) {
                if (searchStudentList.isEmpty()) {
                    try {
                        pageNum = (int) Math.ceil((double) controller.getAllStudentsSize() / numOfStudentsOnPage)
                                - 1;
                    } catch (IOException | InterruptedException | ClassNotFoundException e1) {
                        // TODO Auto-generated catch block
                        e1.printStackTrace();
    ...
    public void updateTable() throws UnknownHostException, ClassNotFoundException, IOException, InterruptedException {
        List<Student> students = controller.getStudentPage(pageNum, numOfStudentsOnPage, "false");
        table.removeAll();
        ...
        pagesIndicatorLabel.setText(String.valueOf(pageNum + 1) + "/" + (int) Math.ceil((double) controller.getAllStudentsSize() / numOfStudentsOnPage));
        table.pack();
        this.pack();
        //super.pack();
    }
    ...
    public void updateTable(List<Student> studentsList)
        ...
        List<Student> students = controller.getStudentPage(pageNum, numOfStudentsOnPage, "true");
        table.removeAll();
        for (Student student : students) {
            TableItem item = new TableItem(table, SWT.NONE);
